...
5. No serviÃ§o **frontend**, abra **Domains** e ligue o domÃ­nio `app.seudominio.com` apontando para a **porta 80** do container. Se o EasyPanel mostrar 502 usando `80`, altere o dominio para `8080` e, se ainda assim persistir, use `3000`; o frontend escuta nessas portas para evitar erro de proxy. O EasyPanel emite o SSL (Let's Encrypt) automaticamente.
...
- **Domains:** ligue `app.seudominio.com` no servico **frontend**. Use porta `80`; se aparecer 502 no dominio, troque para `8080` ou `3000`. SSL automÃ¡tico.
